...
for pf in "${pf[@]}"; do
    for period in "${periods[@]}"; do
        for alpha in "${alphas[@]}"; do
            ...
            if [ "$MODE" == "cpu" ]; then
                bash submit.sh cpu gloo "$pf" "$period" "$alpha" "$HIT_RATE" "$DATASET_NAME" "$NUM_NODES" "$MODEL" "$QUEUE"
            elif [ "$MODE" == "gpu" ]; then
                bash submit.sh gpu nccl "$pf" "$period" "$alpha" "$HIT_RATE" "$DATASET_NAME" "$NUM_NODES" "$MODEL" "$QUEUE"
            else
