            ...
            for SAMPLER_PROCESSES in $NUM_SAMPLER_PROCESSES; do
                for TRAINERS in $NUM_TRAINERS; do
                    ...
                    if [ "$MODE" == "gpu" ]; then
                        ...
                        $NODES $SAMPLER_PROCESSES $SUMMARYFILE $IP_CONFIG_FILE $TRAINERS $BACKEND $EVICTION_PERIOD $PREFETCH_FRACTION $ALPHA $HIT_RATE \
                        $MODEL $PROJ_PATH $PARTITION_DIR $AWS_ACCESS_KEY_ID $AWS_SECRET_ACCESS_KEY $AWS_SESSION_TOKEN"
                    elif [ "$MODE" == "cpu" ]; then
                        CMD="sbatch -N $NODES -q $QUEUE --job-name $JOBNAME -o $OUTFILE -e $ERRFILE --time=$TIME $SCRIPT $DATASET $PARTITION \
